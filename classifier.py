...
class BertSentClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        #output = self.bert.encode(embedding_output, attention_mask=attention_mask)

        out_dict = self.bert(input_ids, attention_mask)

        output = self.dropout(out_dict["pooler_output"])
        pooled_output = self.classifier(output)

        #loss = torch.nn.NLLLoss()
        logits = F.log_softmax(pooled_output, dim=1)
        #logits = m(pooled_output)

        #labels = torch.Tensor([label[1] for label in input_ids])
        #print(labels)
