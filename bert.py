...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    attention = torch.div(qk, math.sqrt(ahs))
    ...
    return torch.reshape(attention, (bs, seqlen, nheads * ahs))
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    """
    # todo
    norm_output = dense_layer(output)
    norm_output = dropout(norm_output) + input
    norm_output = ln_layer(norm_output)
    return norm_output
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    hs_size = hidden_states.shape
    # todo
    # multi-head attention w/ self.self_attention
    output = self.self_attention.forward(hidden_states, attention_mask)
    # add-norm layer
    norm_out = self.add_norm(hidden_states, output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    # feed forward
    #interm_output = norm_out
    interm_output = self.interm_dense(norm_out)
    interm_output = self.interm_af(interm_output)
    # another add-norm layer
    norm2_out = self.add_norm(norm_out, interm_output, self.out_dense, self.out_dropout, self.out_layer_norm)
    return norm2_out.view(hs_size)
